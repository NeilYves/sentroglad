...
// Sanitize function (if not defined in config.php)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $purok_id = $_POST['purok_id'] ?? null; // from dropdown
    $address = sanitize_input($_POST['address'] ?? '');
    $head_of_household_id = $_POST['head_of_household_id'] ?: null;
    $contact_number = sanitize_input($_POST['contact_number'] ?? '');

    if (empty($household_name) || empty($purok_id)) {
        header("Location: household_form.php?action=$action&status=error&message=Household name and Purok are required.");
        exit;
    }
    ...
    if ($action == 'add') {
        ...
        $stmt = mysqli_prepare($link, 
            "INSERT INTO households (household_name, purok_id, address, head_of_household_id, contact_number) 
             VALUES (?, ?, ?, ?, ?)"
        );
        mysqli_stmt_bind_param($stmt, "sisis", $household_name, $purok_id, $address, $head_of_household_id, $contact_number);
        ...
        $stmt = mysqli_prepare($link, 
            "UPDATE households 
             SET household_name = ?, purok_id = ?, address = ?, head_of_household_id = ?, contact_number = ? 
             WHERE id = ?"
        );
        mysqli_stmt_bind_param($stmt, "sissii", $household_name, $purok_id, $address, $head_of_household_id, $contact_number, $household_id);
