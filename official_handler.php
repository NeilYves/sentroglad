...
// Define positions that have limits (only one person can hold these positions at a time)
function getPositionLimits() {
    return [
        // Executive positions - only one person can hold these
        'Barangay Captain' => 1,
        'Punong Barangay' => 1,
        
        // Administrative positions - only one person can hold these  
        'Barangay Secretary' => 1,
        'Barangay Treasurer' => 1,
        
        // SK positions - only one chair
        'SK Chairperson' => 1,
        
        // Note: Barangay Kagawad and specific committee kagawads can have multiple people
        // so they are not included in this list
    ];
}

// --- Helper Function to Handle Position Limits ---
function handlePositionLimits($new_position, $link, $exclude_official_id = null) {
    // Apply position limits to ALL positions - only one person can hold any specific position at a time
    // When adding someone to a position, existing officials with the same position become "Ex-"
    
    $limit = 1; // All positions have a limit of 1 person
    
    // Count current officials with this exact position (excluding those already with Ex- prefix)
    // Also exclude the current official being edited if we're in edit mode
    $count_sql = "SELECT COUNT(*) as count FROM officials WHERE position = ? AND position NOT LIKE 'Ex-%'";
    $params = [$new_position];
    $param_types = "s";
    
    if ($exclude_official_id !== null) {
        $count_sql .= " AND id != ?";
        $params[] = $exclude_official_id;
        $param_types .= "i";
    }
    ...
    if ($count_stmt = mysqli_prepare($link, $count_sql)) {
        ...
        // If we're at or over the limit, move existing officials to "Ex-" status
        if ($current_count >= $limit) {
            $ex_position = "Ex-" . $new_position;
            
            // Update existing officials to Ex- status (excluding the current official being edited)
            $update_sql = "UPDATE officials SET position = ? WHERE position = ? AND position NOT LIKE 'Ex-%'";
            $update_params = [$ex_position, $new_position];
            $update_param_types = "ss";
            
            if ($exclude_official_id !== null) {
                $update_sql .= " AND id != ?";
                $update_params[] = $exclude_official_id;
                $update_param_types .= "i";
            }
            
            if ($update_stmt = mysqli_prepare($link, $update_sql)) {
                mysqli_stmt_bind_param($update_stmt, $update_param_types, ...$update_params);
                
                if (mysqli_stmt_execute($update_stmt)) {
                    $affected_rows = mysqli_stmt_affected_rows($update_stmt);
                    mysqli_stmt_close($update_stmt);
                    
                    // Log the position transition
                    if ($affected_rows > 0) {
                        $log_desc = "Moved {$affected_rows} official(s) from '{$new_position}' to '{$ex_position}' due to new appointment";
                        $log_sql = "INSERT INTO activities (activity_description, activity_type) VALUES (?, 'Position Transition')";
...
if ($action == 'add') {
    ...
    if (isset($_POST['fullname'], $_POST['position'])) {
        ...
        // Handle position limits before adding the new official
        if (!handlePositionLimits($position, $link)) {
            header("Location: manage_officials.php?status=error_position_limit");
            exit;
        }
...
// --- Process Action: EDIT OFFICIAL --- 
    ...
    if (isset($_POST['official_id'], $_POST['fullname'], $_POST['position'])) {
        ...
        // Handle position limits before updating the official (exclude current official from the count)
        if (!handlePositionLimits($position, $link, (int)$official_id)) {
            header("Location: manage_officials.php?status=error_position_limit");
            exit;
        }
