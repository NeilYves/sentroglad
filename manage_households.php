...
if (!empty($search_query)) {
    $where_clauses[] = "(h.household_name LIKE ? OR p.purok_name LIKE ?)";
    $params[] = "%" . $search_query . "%";
    $params[] = "%" . $search_query . "%";
    $param_types .= 'ss';
}
...
$total_households_query = "SELECT COUNT(h.id) AS total FROM households h LEFT JOIN puroks p ON h.purok_id = p.id {$where_sql}";
...
// Main households query
$households_query = "
    SELECT h.id, h.household_name, h.contact_number, 
           p.purok_name,
...
<div class="card mb-4">
    ...
    <div class="card-body">
        <form method="GET" action="manage_households.php" class="row g-3 align-items-end">
            <div class="col-md-10">
                <label for="search" class="form-label">Search Household Name or Purok</label>
                <input type="text" class="form-control" id="search" name="search" value="<?php echo html_escape($search_query); ?>" placeholder="e.g., Household Name, Purok">
            </div>
            <div class="col-md-2">
            ...
            <table class="table table-hover table-striped" id="householdsTable">
                ...
                <?php if ($households_result && mysqli_num_rows($households_result) > 0): ?>
                    ...
                    <tr>
                        ...
                        <td><?php echo html_escape($household['purok_name'] ?? 'N/A'); ?></td>
...
<script>
    $(document).ready(function() {
        ...
        $('.delete-household-btn').on('click', function() {
            ...
            $('#confirmDeleteHouseholdBtn').off('click').on('click', function(e) {
                e.preventDefault();
                $(this).html('<i class="fas fa-spinner fa-spin"></i> Deleting...');
                $(this).prop('disabled', true);
                window.location.href = 'household_handler.php?action=delete&id=' + householdId;
            });
