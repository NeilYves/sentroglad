        ...
        <form action="resident_handler.php" method="POST" id="residentForm">
            ...
            <div class="row mb-3">
                <div class="col-md-3">
                    ...
                    <div id="otherGenderDiv" class="mt-2" style="display: <?php echo ($gender == 'Other') ? 'block' : 'block'; ?>; visibility: <?php echo ($gender == 'Other') ? 'visible' : 'hidden'; ?>; position: <?php echo ($gender == 'Other') ? 'static' : 'absolute'; ?>; left: <?php echo ($gender == 'Other') ? 'auto' : '-9999px'; ?>;">
                ...
                <div class="col-md-3" id="spouseDiv" style="display: <?php echo ($civil_status=='Married')?'block':'none'; ?>;">
                    <label for="spouse_name" class="form-label">Spouse Name</label>
                    <input type="text" class="form-control" id="spouse_name" name="spouse_name" value="<?php echo html_escape($spouse_name); ?>">
                </div>
                <div class="col-md-3">
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="tel" class="form-control" id="contact_number" name="contact_number"
                           value="<?php echo html_escape($contact_number); ?>"
                           placeholder="09XXXXXXXXX"
                           pattern="09[0-9]{9}"
                           maxlength="11"
                           title="Phone number must be 11 digits and start with 09">
                    <small class="form-text text-muted">Must be 11 digits starting with 09 (e.g., 09123456789)</small>
                </div>
            </div>
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-4">
                    ...
                    <select class="form-select" id="maintenance_medicine" name="maintenance_medicine">
                        <option value="">No Maintenance Medicine</option>
                        ...
                        <option value="Heart Disease" <?php echo ($maintenance_medicine == 'Heart Disease') ? 'selected' : ''; ?>>Heart Disease</option>
                        <option value="Asthma" <?php echo ($maintenance_medicine == 'Asthma') ? 'selected' : ''; ?>>Asthma</option>
...
<style>
/* Enhanced phone number validation styles */
#contact_number.is-valid {
    border-color: #28a745;
    background-image: url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 8 8'%3e%3cpath fill='%2328a745' d='m2.3 6.73.8-.77-.8-.77-.8.77.8.77zm1.48-4.97L6.06 4.04l-.8.77L2.98 2.53l-.8-.77.8-.77zm0 0'/%3e%3c/svg%3e");
    background-repeat: no-repeat;
    background-position: right calc(0.375em + 0.1875rem) center;
    background-size: calc(0.75em + 0.375rem) calc(0.75em + 0.375rem);
}

#contact_number.is-invalid {
    border-color: #dc3545;
...
.phone-validation-info.valid {
    color: #28a745;
}

.phone-validation-info.invalid {
    color: #dc3545;
}
</style>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function toggleGenderOther() {
            if (genderSelect.value === 'Other') {
                otherGenderDiv.style.visibility = 'visible';
                otherGenderDiv.style.position = 'static';
                otherGenderDiv.style.display = 'block';
                otherGenderInput.required = true;
            } else {
                otherGenderDiv.style.visibility = 'hidden';
                otherGenderDiv.style.position = 'absolute';
                otherGenderDiv.style.left = '-9999px';
                otherGenderDiv.style.display = 'block';
                otherGenderInput.required = false;
                // Don't clear the value to ensure it's always submitted
            }
        }
        ...
        // Reference to civil status select
        const civilSelect = document.getElementById('civil_status');

        // Handle Civil Status change for spouse field
        const spouseDiv = document.getElementById('spouseDiv');
        const spouseInput = document.getElementById('spouse_name');
        function toggleSpouse() {
            if (civilSelect.value === 'Married') {
        ...
        civilSelect.addEventListener('change', toggleSpouse);
        toggleSpouse();
        ...
        contactNumberInput.addEventListener('input', function(e) {
            ...
            // Ensure it starts with 09 if user types something else
            if (value.length > 0 && !value.startsWith('09')) {
                // If user starts typing and it doesn't start with 09, prepend 09
        ...
        residentForm.addEventListener('submit', function(event) {
            ...
            if (contactNumberInput && contactNumberInput.value.trim() !== '') {
                ...
                if (!/^09\d{9}$/.test(phoneNumber)) {
                    ...
                    const feedback = `<div class="invalid-feedback">Phone number must be exactly 11 digits and start with 09 (e.g., 09123456789).</div>`;
